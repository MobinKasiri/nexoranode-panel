...
export function ReferralSettingsEditor() {
  ...
  const load = () => {
    api
      .get<ReferralData>("/settings/referral")
      .then(setData)
    ...
    <div className="space-y-6 max-w-3xl">
      <Card className="p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-text-muted block mb-1">پاداش معرف (هر خرید زیرمجموعه) — تومان</label>
            <Input
              type="number"
              className="font-latin"
              value={data.referrer_bonus_toman}
              onChange={(e) =>
                setData({ ...data, referrer_bonus_toman: parseInt(e.target.value, 10) || 0 })
              }
            />
          </div>
          <div>
            <label className="text-xs text-text-muted block mb-1">نوع هدیه کاربر دعوت‌شده</label>
            <select
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setData({
                      ...data,
                      friend_welcome: { ...data.friend_welcome, percent: parseInt(e.target.value, 10) || 0 },
                    })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setData({
                      ...data,
                      friend_welcome: { ...data.friend_welcome, valid_days: parseInt(e.target.value, 10) || 1 },
                    })
                  }
                />
              </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setData({
                    ...data,
                    friend_welcome: { ...data.friend_welcome, toman: parseInt(e.target.value, 10) || 0 },
                  })
                }
              />
            </div>
      ...
      <Card className="p-6 space-y-4">
        ...
        {(["landing", "ready_post"] as const).map((slot) => (
          <div key={slot} className="flex flex-col sm:flex-row gap-3 items-start sm:items-center border border-border/60 rounded-lg p-3">
